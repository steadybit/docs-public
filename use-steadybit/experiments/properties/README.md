...
In general, a user can only assign properties to an experiment design or run that have been [defined by an administrator](../../../install-and-configure/manage-properties/#manage-property-definitions). Once you - or your administrator - has assigned the property, you can capture values per experiment design or experiment run.
...
Property values are the actual values of the properties that are saved as part of an experiment design and copied to each experiment run. [Variables](../variables.md) are fully supported for the input fields of all properties. (including overrides for single runs or via schedules).
...
Properties' values can also be read and edited using the following API endpoints. Check out [Integrate with Steadybit / API](../../../integrate-with-steadybit/api/api.md) how to access the API.
