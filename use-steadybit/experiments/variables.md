...
![An experiment-scoped variable shadowing an environment-scoped variable](../../.gitbook/assets/variable-experiment-shadow.png)
...
For a single experiment run, you can override the variables it uses. The values you provide apply to that one run only. To do so, access the modal through the `Run Experiment` button in the editor.

![How to access experiment overrides](variables-run-experiment-split-button.png)

Doing so will open a modal where you can specify new values for each used variable. Experiment variable overrides will only be applied for this specific run. Once the run has been triggered, the overrides are gone and the environment, service and experiment values apply again.
...
Referenced variables are resolved with the same [resolution strategy](#resolution-strategy) and scope rules as everywhere else, so a reference can point to a variable from any scope (environment, service, experiment, or a run override).
