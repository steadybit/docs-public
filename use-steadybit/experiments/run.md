...
| COMPLETED | Entire experiment including all actions were successfully executed - so, no failure or error reported by any step.                                                                                                                                                        |
...
| COMPLETED | The step's action was successfully executed on all targets - so, no failure or error.                                                                                                                     |
| CANCELED  | The step was running before and got now canceled, either by a user canceling the entire experimentr un or by the system (e.g. when another step running in parallel caused the experiment to stop early). |
| SKIPPED   | The step was never executed, because the experiment was stopped before.                                                                                                                                   |
| FAILED    | The step's action noticed a failed check, for example, a _HTTP Check_ not reaching the required success rate.                                                                                             |                                                                                                                                                      |
| ERRORED   | The step's action errored due to some technical reasons like `I/O error on POST request: Connection refused` or `Agent disconnected unexpectedly`.                                                        |
...
| PREPARED  | The agent having discovered the selected target is connected.                                                                                                                                                                                                            |                                                                                                                                                    |
| RUNNING   | The action is currently executed on this selected target.                                                                                                                                                                                                                |
| COMPLETED | The action was successfully executed on this target - so, no failure or error.                                                                                                                                                                                           |
| CANCELED  | The action was executed before and got now canceled on this target, either by a user canceling the entire experiment run or by the system (e.g. when another target execution of the same step or another step running in parallel caused the experiment to stop early). |
| SKIPPED   | The action was never executed on this target, because the experiment's step was stopped before.                                                                                                                                                                          |
| FAILED    | The action performed on this target noticed a deviation in the check, for example, a _HTTP Check_ not reaching the required success rate.                                                                                                                                |
| ERRORED   | The action performed on this target errored due to some technical reasons like `I/O error on POST request: Connection refused` or `Agent disconnected unexpectedly`.                                                                                                     |

### State Propagation

The different levels of states (**experiment run**, **step**, **target execution**) can be propagated to higher levels (e.g., state `FAILED` at target execution level is propagated to step being `FAILED` and experiment run being `FAILED`).
But also change the states of adjacent instances of the same level (i.e., a `FAILED` step causes subsequent steps to be `SKIPPED`) and level below (i.e., for a `SKIPPED` step all target executions are in the `SKIPPED` state as well).
...
4. Steps running in parallel **(2)** are canceled (step's state `CANCELED`, thus all it's target execution states are `CANCELED`)
...
In some cases, the state propagation is not desirable.
Especially, when running an experiment that attacks a large amount of targets, single targets becoming unavailable may be expected and shouldn't cause the experiment to stop immediately.
For these cases, you can configure in the experiment design a step to [continue on any failures or errors](./design.md).
...
5. Subsequent steps **(3)** are performed as usual, if no failure or error occurs, the steps' and target execution states' are `COMPLETED`
6. Steps already performed **(4)** beforehand don't change their state (i.e. `COMPLETED`)
