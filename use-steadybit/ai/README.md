...
Steadybit AI is an assistant built into the platform that helps you design, run, and understand chaos experiments using natural language.
Instead of clicking through the experiment editor, you can describe what you want to test, ask why an experiment failed, or let Steadybit AI propose ready-to-run experiments tailored to your environment.

<!-- TODO screenshot: Steadybit AI chat overview / assistant landing page -->


## How Steadybit AI Works

Steadybit AI acts **on your behalf** and only sees what you are allowed to see.
...
- **Your permissions** — Steadybit AI reads the same data you can read in the UI/API. It cannot surface targets, environments, or runs you have no access to.

{% hint style="warning" %}
**Steadybit AI never performs destructive operations on its own.**
...
How Steadybit AI is enabled depends on whether you use the Steadybit SaaS platform or run Steadybit on-prem.

### SaaS — Opt In

For SaaS customers, Steadybit AI is **off by default** and must be explicitly enabled by a tenant administrator.
...
To enable Steadybit AI, access it from the navigation or go to **Settings → Data & Access**.
Once enabled, every user within your tenant can start using Steadybit AI.

If you are not an administrator, ask one of your tenant admins to enable it for you.

<!-- TODO screenshot: SaaS opt-in / introduction page with "Enable Steadybit AI for tenant" -->


### On-Prem — Configure a Provider

For self-hosted (on-prem) installations, there is no separate opt-in: Steadybit AI is available once you configure a model provider in the platform's environment variables and the AI capability is part of your license.
On-prem deployments run against your own AI provider.

See [configuration options / AI](../../install-and-configure/install-on-prem-platform/advanced-configuration.md#steadybit-ai) for the provider, model, and retention settings.

## Data Processing and Privacy

Steadybit AI only sends the context needed to answer your request.
...
{% hint style="info" %}
**SaaS:** Steadybit AI uses **Anthropic** models.
...
On-prem, retention periods are [configurable](../../install-and-configure/install-on-prem-platform/advanced-configuration.md#steadybit-ai).

## What You Can Do

### Get Experiment Suggestions

Pick an environment and Steadybit AI analyzes the targets it contains to propose ready-to-run experiment ideas tailored to what it finds.
...
Steadybit AI asks clarifying questions when needed, then produces an experiment draft.
You can:
...
Steadybit AI reads the relevant run data and explains what happened, grounding its answer in the actual execution results rather than guessing.
