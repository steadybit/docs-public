...
1. Place your Gatling load test file in a separate folder. We assume for this steps that it is named _custom.scala_. Alternatively, download an example [custom.scala](../actions/gatling-custom.scala) load test and adjust it as needed.
2. Put a [Dockerfile](../actions/gatling-Dockerfile/) into the same directory which simply copies the custom load test into the Docker image. It looks like:
...
![Experiment with Custom Gatling Load Test](../actions/gatling-experiment.png)
...
Within the Gatling load test you have access to the following parameters as environment variables. You can use them in the script via e.g. `${__ENV.DURATION}` as shown in the attached [custom.scala](../actions/gatling-custom.scala).
