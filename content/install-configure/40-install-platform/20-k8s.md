...
To install the chart, retrieve your steadybit tenant and agent key from the [setup page](https://platform.steadybit.io/settings/agents/setup) in the SaaS platform and run:
...
helm install steadybit-platform \
  ...
  --set platform.tenant.key=<replace-with-tenant-key> \
  --set platform.tenant.name=<replace-with-your-org-name> \
