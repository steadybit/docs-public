...
| `STEADYBIT_AUTH_LDAP_SYNC_TEAM_KEY_PATTERN`    |          | <p>Regular expression applied to the team key attribute value; the first capture group becomes the team key (uppercased, special characters replaced by <code>_</code>, cut to 16 characters). Teams whose key attribute does not match are not synchronized. Changing the pattern re-keys the synchronized teams; the teams under the old keys are deleted as stale on the next sync.<br><strong>Default:</strong> <code>^(.{0,16})</code><br><strong>Example:</strong> <code>^steadybit-(.+)$</code></p> |
...
| `STEADYBIT_AUTH_OAUTH2_TEAM_KEY_PATTERN`                |          | <p>Regular expression applied to each entry of the team claim; the first capture group becomes the team key (uppercased, special characters replaced by <code>_</code>, cut to 16 characters). Entries that do not match are not synchronized. See <a href="oidc-integration.md#team-key-pattern">Team key pattern</a>.<br><strong>Default:</strong> <code>^(.{0,16})</code><br><strong>Example:</strong> <code>^steadybit-(.+)$</code></p>                |
